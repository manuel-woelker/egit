...
public class BlameEngine {
	...
	public List<BlameEntry> blame(Repository repository, String path) {
		try {
			...
			Scoreboard scoreboard = new Scoreboard(finalOrigin, new JavaDiffImpl());
	...
	/** print it pretty
	 * @param blameEntries entries to print
	 */
	public void prettyPrint(List<BlameEntry> blameEntries) {
		...
		for (BlameEntry blameEntry : blameEntries) {
			...
			for(int i = blameEntry.suspectStart; i < blameEntry.suspectStart+blameEntry.originalRange.length; i++) {
				System.out.println(String.format("%3d: [%.59s] %s", Integer.valueOf(lineno), blameEntry.suspect, data[i]));
				lineno++;
			}
		}
	}
